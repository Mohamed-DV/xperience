...
class PaymentSelector extends ConsumerStatefulWidget {
  final bool forUpdate;
  const PaymentSelector({super.key, this.forUpdate = false});

  @override
  ConsumerState<PaymentSelector> createState() => _PaymentSelectorState();
}

class _PaymentSelectorState extends ConsumerState<PaymentSelector> {
  @override
              ...
              Text(
                ref.watch(reservationRepositoryProvider).selectedType ==
                    ...
                    ? ref
                            ...
                            ?.title ??
                        'Événement'
                    : ref.watch(reservationRepositoryProvider).event?.title ??
                        'Événement',
                  ...
                  (() {
                    final dateStr =
                        ref
                            .watch(reservationRepositoryProvider)
                            .reservationDate;
                    if (dateStr.isEmpty) return 'Non spécifié';
          ...
          children: [
            Column(
              ...
              children: [
                ...
                Text(
                  ref.watch(reservationRepositoryProvider).fromHour.isNotEmpty
                      ? '${ref.watch(reservationRepositoryProvider).fromHour} - ${ref.watch(reservationRepositoryProvider).toHour}'
                      : 'Non spécifié',
                  style: GoogleFonts.montserrat(
                    fontSize: 14,
                    color: AppColors.primaryGrey,
          ...
          children: [
            Column(
              ...
              children: [
                ...
                Text(
                  '${ref.watch(reservationRepositoryProvider).childrenCount + ref.watch(reservationRepositoryProvider).adultsCount} participant${ref.watch(reservationRepositoryProvider).childrenCount + ref.watch(reservationRepositoryProvider).adultsCount > 1 ? 's' : ''}',
                  style: GoogleFonts.montserrat(
                    fontSize: 14,
                    color: AppColors.primaryGrey,
                  ),
  ...
  Widget buildPaymentDetails() {
    final personnes =
        ref.watch(reservationRepositoryProvider).childrenCount +
        ref.watch(reservationRepositoryProvider).adultsCount;

    final totalPrice =
        (double.parse(ref.watch(reservationRepositoryProvider).totalPrice) *
            personnes); // Adding service fee
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
        ...
        Row(
          ...
          children: [
            Text(
              '${double.parse(ref.watch(reservationRepositoryProvider).totalPrice)}€ x ${ref.watch(reservationRepositoryProvider).childrenCount + ref.watch(reservationRepositoryProvider).adultsCount} per.',
              style: GoogleFonts.montserrat(
                fontSize: 14,
        ...
        Row(
          ...
          children: [
            ...
            Text(
              '${(totalPrice + 0.89).toStringAsFixed(2)}€',
              style: GoogleFonts.montserrat(
                fontSize: 14,
                fontWeight: FontWeight.w400,
                color: AppColors.primaryGrey,
  ...
  Widget buildButonConfirmationPayment() {
    final isLoading = ref.watch(reservationRepositoryProvider).isLoading;
    return Column(
      children: [
        Text(
