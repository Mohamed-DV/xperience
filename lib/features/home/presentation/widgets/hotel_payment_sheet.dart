...
class HotelPaymentSheet extends ConsumerStatefulWidget {
  const HotelPaymentSheet({super.key});

  @override
  ConsumerState<HotelPaymentSheet> createState() => _HotelPaymentSheetState();
}

class _HotelPaymentSheetState extends ConsumerState<HotelPaymentSheet> {
  @override
  Widget build(BuildContext context) {
    ref.listen(myhomeNotifierProvider, (pr, state) {
      state.maybeWhen(
        createHotelReservationSuccess: () async {
          showDialog(
            context: context,
  ...
  Widget buildCoverReservation() {
    final resto = ref.watch(detailsHotelModelProvider);
    return Row(
  ...
  Widget buildButonConfirmationPayment() {
    return Column(
      children: [
        ...
        GestureDetector(
          onTap: () {
            final reservation = ref.read(reservationHotelProvider);
            ref
                .read(myhomeNotifierProvider.notifier)
                .createHotelReservation(reservation);
          },
          child: Container(
  ...
  Widget buildPaymentDates() {
    final resto = ref.watch(reservationHotelProvider);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
